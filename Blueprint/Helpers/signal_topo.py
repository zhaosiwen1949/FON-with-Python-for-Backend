...
def generate_signal_topo (node_num):
    if node_num == 3:
        seed = randint(1,2)
        if seed == 1:
            result = gtta(list(c_graph['1_of_3_nodes']))
        elif seed == 2:
            result = gtta(list(c_graph['2_of_3_nodes']))
    elif node_num == 4:
        seed = randint(1,5)
        if seed == 1:
            result = gtta(list(c_graph['1_of_4_nodes']))
        elif seed == 2:
            result = gtta(list(c_graph['2_of_4_nodes']))
        elif seed == 3:
            result = gtta(list(c_graph['3_of_4_nodes']))
        elif seed == 4:
            result = gtta(list(c_graph['4_of_4_nodes']))
        elif seed == 5:
            result = gtta(list(c_graph['5_of_4_nodes']))
    return result
